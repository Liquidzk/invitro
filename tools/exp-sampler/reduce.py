...
def thin_invocations_round_robin(inv_df: pd.DataFrame, factor: float, seed: int | None = None) -> pd.DataFrame:
    ...
    thinned_counts = (counts * factor).astype(np.int64) + rng.binomial(
        1, (counts * factor) - (counts * factor).astype(np.int64), size=counts.shape
    )
...
def thin_invocations_per_function(inv_df: pd.DataFrame, factors: np.ndarray, seed: int | None = None) -> pd.DataFrame:
    ...
    rng = np.random.default_rng(seed)
    thinned_counts = (counts * probabilities).astype(np.int64) + rng.binomial(
        1, (counts * probabilities) - (counts * probabilities).astype(np.int64), size=counts.shape
    )
